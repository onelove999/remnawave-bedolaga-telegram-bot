...
Снимок Grace имеет версию 4 и явно хранит стратегию сброса трафика и поколение `lastTrafficResetAt`. Открытые снимки версий 2–3 по-прежнему читаются, но отсутствующая стратегия остаётся `None` до точного чтения панели; значение не угадывается.

Для SQLite защитные операции берут единый writer lock. SQLite поддерживается для одного экземпляра бота; для нескольких worker-процессов нужен PostgreSQL.

Кандидаты Grace выбираются последовательными страницами по numeric subscription id, поэтому объём памяти не растёт вместе с размером таблицы.
...
- Grace передаёт срок только в UTC и только когда до него остаётся больше 60 секунд: опасный `ACTIVE`/`LIMITED` PATCH отменяется, а безопасный `DISABLED` выполняется без истекающей даты. Производные статусы `LIMITED` и `EXPIRED` никогда не записываются вручную.
